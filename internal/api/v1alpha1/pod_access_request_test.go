...
// The PodAccessRequest tests are primarily testing the behavior of the
// PodAccessRequest struct - but because some of our tests are testing against
// the real Kubernetes API, they indirectly are also acting as tests of the
// PodAccessRequestController Reconciler().
var _ = Describe("PodAccessRequest", Ordered, func() {
	var namespace *corev1.Namespace
	var deployment *appsv1.Deployment
	var template *PodAccessTemplate

	// These tests create real PodAccessRequest{} objects in the cluster and
	...
	Context("Reconciliation / Webhook Tests", func() {
		...
		It("Creation of the PodAccessRequest to work - Passes Webhook", func() {
		...
		It("PodAccessRequest becomes ready - Passes Reconciliation", func() {
			request := &PodAccessRequest{}
		...
		It("PodAccessRequest Update - Passes Webhook ValidateUpdate() Call", func() {
	...
	Context("Functional Unit Tests", func() {
		var (
			...
			gvr = metav1.GroupVersionResource{
				...
				Resource: "podaccessrequest",
			}
			gvk = metav1.GroupVersionKind{
				Group:   api.SchemeGroupVersion.Group,
				Version: api.SchemeGroupVersion.Version,
				Kind:    PodAccessRequest{}.Kind,
			}
		)
	...
	BeforeEach(func() {
		...
		// Create the podaccesstemplate
		template = &PodAccessTemplate{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test",
				Namespace: namespace.Name,
			},
			Spec: PodAccessTemplateSpec{
				...
				ControllerTargetMutationConfig: &PodTemplateSpecMutationConfig{},
			},
		}
